...
func _explode(damage: float) -> void:
	...
	var enemy_areas = explosion_area.get_overlapping_areas()
	for area in enemy_areas:
		var owner = area.get_parent()
		if is_instance_valid(owner) and owner.is_in_group("Enemy") and owner.has_method("take_damage"):
			var owner_id = owner.get_instance_id()
			if not hit_enemy_ids.has(owner_id):
				hit_enemy_ids[owner_id] = true
