...
## 获取无尽模式下的基础属性增强倍率 (线性增长：速度/伤害/经验)
func get_endless_multiplier() -> float:
	if not is_endless_mode: return 1.0
	# 线性平滑增长：每 10 秒增加 5%
	return 1.0 + (endless_time / 10.0) * 0.05

## 获取无尽模式下的血量增强倍率 (二次函数增长：y = 1 + k * x^2)
## 这种增长方式会让前期相对平滑，但后期生存压力陡增
func get_endless_hp_multiplier() -> float:
	if not is_endless_mode: return 1.0
	# 公式：1.0 + (时间/60)^2 * 0.1
	# 这里的 60 是一个时间基准（分钟），0.2 是系数（系数越大中后期血量越高）
	var time_in_min = endless_time / 60.0
	return 1.0 + (time_in_min * time_in_min) * 0.1
